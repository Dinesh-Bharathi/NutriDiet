...
import {
  MEAL_TYPE,
  DIET_PLAN_STATUS,
} from "../diet-plans/diet-plan.constants.js";
...
export const createTemplateMealSchema = z.object({
  ...
  body: z.object({
    name: mealTypeEnum,
    mealOrder: z
      .number({ required_error: "Meal order is required" })
      .int()
      .min(1),
...
// Template Meal Items
export const createTemplateMealItemSchema = z
  .object({
    params: z.object({
      mealId: z.string().min(1, "Template Meal ID is required"),
    }),
  ...
  .refine(
    ...
    {
      message:
        "Food name and unit are required when not selecting from Food Library",
      path: ["body.foodName"],
    },
  );
